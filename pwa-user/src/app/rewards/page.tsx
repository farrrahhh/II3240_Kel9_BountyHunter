  ...
  return (
    <div className="flex h-screen w-full max-w-[1440px] mx-auto bg-[#221E1E] font-[Inter] text-white">
      <aside className="w-64 bg-[#111111] h-full flex flex-col justify-between">
        ...
        <div className="p-6 border-t border-gray-800 cursor-pointer hover:opacity-80 transition" onClick={() => router.push("/users")}> 
      ...
      <main className="flex-1 bg-[#1a1a1a] overflow-y-auto">
        ...
        <section className="relative h-[200px] px-6 py-8 text-white overflow-hidden">
          <div className="absolute inset-0 w-full h-full">
            <Image src="/picture/image5.png" alt="Reward Hero Background" fill className="object-cover" style={{ filter: "blur(0px)", transform: "scale(1.05)" }} priority />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.2), rgba(0,0,0,0.4))", zIndex: 1 }} />
          </div>
          <div className="relative z-10">
            <h1 className="text-3xl font-bold text-black">Turn Trash Into Treasure!</h1>
            <p className="mt-2 font-bold text-sm max-w-xl text-black">
              Earn Reward Points By Recycling At Smart Bins And Redeem Them For Exciting Discounts, Vouchers, And Exclusive Deals!
            </p>
            <div className="absolute top-6 right-6 text-right">
              <h2 className="text-4xl font-bold text-black">{points}</h2>
        ...
        <section className="px-6 py-12">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <button onClick={() => router.push("/rewards_history")} className="bg-[#8BC34A] hover:bg-[#7ab63e] text-white px-4 py-2 rounded-md font-medium transition-colors">
            ...
            {rewards.map((reward) => {
              ...
              return (
                <div key={reward.reward_id} className="bg-[#2a2a2a] rounded-md p-4 flex flex-col sm:flex-row sm:items-center justify-between">
                  ...
                  <button onClick={() => { setSelectedReward(reward); setShowConfirm(true); }} disabled={disabled} className={`mt-4 sm:mt-0 px-6 py-2 rounded font-semibold ${disabled ? "bg-gray-600 text-gray-400 cursor-not-allowed" : "bg-[#8BC34A] text-white hover:bg-[#7ab63e]"}`}>
        ...
        {showConfirm && selectedReward && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
            <div className="bg-white text-black rounded-md shadow-md w-[90%] max-w-md p-6">
              <h3 className="text-xl font-bold mb-4">Konfirmasi Penukaran</h3>
              <p className="mb-4">
                Apakah Anda yakin ingin menukar <strong>{selectedReward.name}</strong> dengan <strong>{selectedReward.point_cost}</strong> poin?
              </p>
              <div className="flex justify-end gap-3">
                <button className="bg-gray-300 hover:bg-gray-400 text-black px-4 py-2 rounded" onClick={() => setShowConfirm(false)}>
                  Batal
                </button>
                <button className="bg-[#8BC34A] hover:bg-[#7ab63e] text-white px-4 py-2 rounded font-semibold" onClick={handleRedeem}>
                  Ya, Tukar
                </button>
              </div>
            </div>
        ...
        {successMessage && (
          <div className="fixed bottom-4 right-4 bg-green-600 text-white px-6 py-3 rounded shadow-lg z-50">
            {successMessage}
            <button className="ml-4 font-bold" onClick={() => setSuccessMessage("")}>×</button>
          </div>
        )}
      </main>
    </div>
